...
class Fitbit(object):
    ...
    def activity_logs_list(self, user_id=None, before_date=None,after_date=None, limit=20):

        if not ((isinstance(before_date,str) and after_date is None) or
                (isinstance(after_date,str)and before_date is None)) :
            raise ValueError ("before_date 또는 after_date가 필요합니다")
        if isinstance(before_date,str) :
            sort = "desc"
        else :
            sort = "asc"
        url = "{0}/{1}/user/{2}/activities/list.json".format(*self._get_common_args(user_id))
        ...
        return self.make_request(url,params=collect)
