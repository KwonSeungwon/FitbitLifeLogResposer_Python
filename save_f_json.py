...
while True :
    ...
    fitbit_stats = auth2_client.intraday_time_series('activities/heart', base_date=now, detail_level='1sec',start_time="15:00",end_time="16:00")
    stats = fitbit_stats
    print(stats)

    with open('c:\HR\Heratrate' + date+ '.json', 'w') as outfile:
        json.dump(stats, outfile, indent=4)

    fitbit_stats2 = auth2_client.intraday_time_series('activities/steps', base_date=now, detail_level='1min',start_time="15:00",end_time="16:00")
    stats2 = fitbit_stats2
    print(stats2)


    with open('c:\STEPS\Steps' + date+ '.json', 'w') as outfile:
